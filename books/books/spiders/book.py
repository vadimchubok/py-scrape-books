...
class BooksSpider(scrapy.Spider):
    ...
    def _parse_catalog(self, response: Response) -> Generator[scrapy.Request, None, None]:
    ...
    def _paginate(self, response: Response) -> Generator[scrapy.Request, None, None]:
        next_page = response.css("li.next a::attr(href)").get()
        if next_page:
            yield response.follow(next_page, callback=self.parse)


    def parse_book(self, response: Response) -> Generator[Dict[str, Any], None, None]:
    ...
    def _extract_stock(self, response: Response) -> int | None:
    ...
    def _extract_rating(self, response: Response) -> int:
        classes = response.css("p.star-rating::attr(class)").get(default="")
    ...
    def _extract_category(self, response: Response) -> str | None:
        return response.css("ul.breadcrumb li:nth-last-child(2) a::text").get()
